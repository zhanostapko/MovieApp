...
const showMovies = function (data) {
    ...
    data.forEach((movie)=>{
        const {title, vote_average: raiting, overview, poster_path: img }=movie;

        const raitingColor = function(rate) {
        ...
        <div class="movie-info">
          <h3>${title}</h3>
          <span class="${raitingColor(raiting)}">${raiting}</span>
        </div>
        <div class="overview">
          <h3>Overview</h3>
          ${overview}
...
form.addEventListener('submit', (e) => {
    ...
    if(search.value && search.value !== '') {
        getMovies(`https://api.themoviedb.org/3/search/movie?&api_key=3fd2be6f0c70a2a598f084ddfb75487c&query=${search.value}`);
